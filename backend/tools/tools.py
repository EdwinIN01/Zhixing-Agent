...
def decode_amap_polyline(encoded):
    """
    解析高德地图 polyline 字符串为 [[lng, lat], ...] 格式。
    高德驾车路线 API 返回两种 polyline 格式，本函数自动识别：
    1. "lng,lat;lng,lat;..."  - 分号分隔坐标字符串（path 级别）
    2. 标准 polyline 编码格式    - 类似 Google Maps 的压缩编码（部分场景）
    """
...
def _try_parse_coord(text):
    """如果传入的是纯坐标字符串（如 "116.3974,39.9091"），直接返回。否则返回 None。"""
...
async def simple_geocode(address):
    ...
    支持两种输入：
    1. 纯坐标字符串（如 "116.3974,39.9091"）→ 直接透传，不调用 API
    2. 地址文本 → 调用高德地理编码 API
    """
    parsed = _try_parse_coord(address)
    if parsed:
        logger.info(f"地理编码: 识别为纯坐标 '{address}' → {parsed}")
        return parsed
...
def _parse_driving_response(raw_response):
    ...
    for path in routes_data:
        ...
        decoded = decode_amap_polyline(polyline)
