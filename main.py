...
async def read_root():
    ...
    if os.path.exists(index_path):
        with open(index_path, "r", encoding="utf-8") as f:
            html = f.read()
        # 注入高德地图 JS API Key
        amap_js_key = os.getenv("AMAP_JS_KEY") or os.getenv("AMAP_KEY", "")
        html = html.replace('window.AMAP_JS_KEY = ""', f'window.AMAP_JS_KEY = "{amap_js_key}"')
...
async def stream_response(session_id: str, user_query: str, image_base64: Optional[str] = None):
    ...
    try:
        ...
        # 附加路线数据供前端绘制地图
        end_data = {"type": "end", "content": "", "session_id": session_id}
        if isinstance(result, dict) and result.get("candidate_routes"):
            end_data["routes"] = result["candidate_routes"]
